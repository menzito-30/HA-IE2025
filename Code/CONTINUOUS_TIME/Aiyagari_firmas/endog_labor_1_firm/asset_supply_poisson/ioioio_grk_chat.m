...
for ir = 1:Ir
    ...
    % Precompute boundary labor for amin state constraint
    ell_min = zeros(1,2);
    c_min = zeros(1,2);
    dV_min = zeros(1,2);
    for j = 1:2
        ...
        if isnan(ell_min(j))
            warning('fzero failed at amin boundary for state %d', j);
            ell_min(j) = 1;
        end
    ...
    % Precompute boundary for amax (zero-drift consistent with endogenous labor)
    ell_max = zeros(1,2);
    c_max = zeros(1,2);
    dV_max = zeros(1,2);
    for j = 1:2
        income_max = @(l) w*z(j)*l + r*amax;
        eq_max = @(l) l^(1/Frisch) * (income_max(l))^ga - w*z(j);
        ell_max(j) = fzero(eq_max, 1, optimset('Display','off','TolFun',1e-10));
        if isnan(ell_max(j))
            warning('fzero failed at amax boundary for state %d', j);
            ell_max(j) = 1;
        end
        c_max(j) = income_max(ell_max(j));
        dV_max(j) = c_max(j)^(-ga);
    end
    ...
    for n = 1:maxit
        ...
        % At amax: Use zero-drift endogenous labor (fixed)
        dVf(I,:) = dV_max;
        ...
        for j = 1:2
            for i = 1:I
                ...
                if isnan(ell_f(i,j))
                    warning('fzero failed for forward ell at i=%d, j=%d', i, j);
                    ell_f(i,j) = 1;
                end
            end
        end
        ...
        for j = 1:2
            for i = 1:I
                ...
                if isnan(ell_b(i,j))
                    warning('fzero failed for backward ell at i=%d, j=%d', i, j);
                    ell_b(i,j) = 1;
                end
            end
        end
        ...
        for j = 1:2
            for i = 1:I
                ...
                if isnan(ell_0(i,j))
                    warning('fzero failed for zero-drift ell at i=%d, j=%d', i, j);
                    ell_0(i,j) = 1;
                end
                c0(i,j) = income_0(ell_0(i,j));
            end
        end
        dV0 = max(real(c0 .^ (-ga)), 1e-10);  % Ensure positive and real
        ...
        % Use upwind-consistent drift for robustness
