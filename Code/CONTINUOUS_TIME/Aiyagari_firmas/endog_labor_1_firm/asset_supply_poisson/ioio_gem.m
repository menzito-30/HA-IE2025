...
s = 2;            % Coeficiente de aversión relativa al riesgo (\gamma)
rho = 0.05;       % Tasa de descuento subjetiva
Frisch = 1.0;     % NUEVO: Elasticidad de Frisch (\varphi)
la = [la1,la2];
...
% --- NUEVO: Pre-cálculo de la solución con s=0 (Drift nulo) ---
% Esto reemplazará las antiguas líneas c0 y dV0 en el bucle HJB
l0 = zeros(I,2);
c0 = zeros(I,2);
params = zeros(6,1);
params(1) = amin; % 'a' se actualizará
params(3) = w;
params(5) = s;     % 'ga' (gamma) en lab_solve
params(6) = Frisch;% 'Frisch' (\varphi) en lab_solve

options = optimset('Display','off','TolFun',1e-12);

for i=1:I
    params(1) = a(i); % 'a' actual
    
    % Resolver para z1
    params(2) = z(1); % 'z' actual
    l0(i,1) = fsolve(@(l) lab_solve_aiyagari(l,params), 0.5, options);
    
    % Resolver para z2
    params(2) = z(2); % 'z' actual
    l0(i,2) = fsolve(@(l) lab_solve_aiyagari(l,params), 0.5, options);
end

c0 = w*zz.*l0 + r.*aa; % c = wzl + ra
dV0 = c0.^(-s); % v' = c^{-\gamma}
