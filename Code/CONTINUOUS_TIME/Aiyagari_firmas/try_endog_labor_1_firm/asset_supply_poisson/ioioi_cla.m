...
% Average productivity (for labor supply normalization)
...
for ir = 1:Ir
    ...
    for n = 1:maxit
        ...
        for j = 1:2
            % Solve for labor at upper boundary
            params_upper = [amax, z(j), w, r, ga, Frisch];
            ell_upper = fzero(@(l) lab_solve_aiyagari(l, params_upper), 1);
            income_upper = w*z(j)*ell_upper + r*amax;
            dVf(I,j) = income_upper^(-ga);
        end
        
        % At amin: Solve for labor when drift = 0
        for j = 1:2
            params_lower = [amin, z(j), w, r, ga, Frisch];
            ell_lower = fzero(@(l) lab_solve_aiyagari(l, params_lower), 1);
            income_lower = w*z(j)*ell_lower + r*amin;
            dVb(1,j) = income_lower^(-ga);
        end
        
        % --- 5.3. UPWIND SCHEME WITH ENDOGENOUS LABOR ---
        
        % Forward difference
        cf = dVf.^(-1/ga);
        % Solve for labor using intratemporal condition
        ell_f = zeros(I, 2);
        for j = 1:2
            for i = 1:I
                % Solve: ℓ^(1/φ) * c^γ = w*z
                % Using: c = (dVf)^(-1/γ)
                ell_guess = 1;
                ell_f(i,j) = fzero(@(l) l^(1/Frisch)*cf(i,j)^ga - w*z(j), ...
                                   ell_guess, optimset('Display','off'));
            end
        end
        ...
        for j = 1:2
            for i = 1:I
                ell_guess = 1;
                ell_b(i,j) = fzero(@(l) l^(1/Frisch)*cb(i,j)^ga - w*z(j), ...
                                   ell_guess, optimset('Display','off'));
            end
        end
        ...
        % Zero drift
        % When drift = 0, income = consumption
        % Need to solve jointly: c = w*z*ℓ + r*a and ℓ^(1/φ)*c^γ = w*z
        c0 = zeros(I, 2);
        ell_0 = zeros(I, 2);
        for j = 1:2
            for i = 1:I
                ...
                ell_0(i,j) = fzero(eq_0, 1, optimset('Display','off'));
        ...
        A1 = spdiags(Y(:,1), 0, I, I) + spdiags(X(2:I,1), -1, I, I) + ...
             spdiags([0; Z(1:I-1,1)], 1, I, I);
        A2 = spdiags(Y(:,2), 0, I, I) + spdiags(X(2:I,2), -1, I, I) + ...
    ...
    % Display progress
    if mod(ir, 10) == 0
        fprintf('Completed r iteration %d/%d, S = %.4f\n', ir, Ir, S(ir))
    end
...
% Save figure
print('-dpng', '/mnt/user-data/outputs/aiyagari_endog_labor_equilibrium.png', '-r300')

% =========================================================================
% 8. SAVE RESULTS
% =========================================================================

save('/mnt/user-data/outputs/aiyagari_endog_labor_results.mat', ...
     'r_grid', 'S', 'KD', 'w_r', 'V_r', 'g_r', 'c_r', 'ell_r', ...
     'adot', 'a', 'z', 'ga', 'Frisch', 'rho', 'al', 'd')
